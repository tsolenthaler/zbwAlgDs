...
- **Aufgabe:** 
    ```
    Erstelle mir eine Aufgabe welche eine Komplexitätsklasse von O(1), O(n), O(log(n)), O(n log(n)), O(n^2) oder O(n^k) hat und ich die Komplexität bei einem C# Beispiel berechnen muss.
    ```